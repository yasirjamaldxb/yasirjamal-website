...
async function captureDubaiPodiatristClean() {
  console.log('Launching Chrome to capture clean Dubai Podiatrist Clinic (https://dubaipodiatrist.com/)...');
  ...
  const targetUrl = 'https://dubaipodiatrist.com/';
  ...
  // Smoothly scroll down the entire page to trigger all lazy-loaded medical graphics and clinical directories
  ...
  console.log('Waiting 30 seconds for all clinical directories, doctor profiles, and graphics to fully render...');
  ...
  console.log('Dubai Podiatrist Clinic page ready for clean screenshot:', pageInfo);

  const outputPath = path.resolve('public/images/portfolio/dubai-podiatrist_full.jpg');
  const mainPath = path.resolve('public/images/portfolio/dubai_podiatrist.jpg');
  ...
  console.log('✓ Successfully captured 100% clean Dubai Podiatrist Clinic full-page screenshot!');
  await browser.close();
}

captureDubaiPodiatristClean().catch((err) => {
